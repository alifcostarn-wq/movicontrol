...
function extrairWifi(reg, contexto) {
  const ssids = [], senhas = [];
...
function linhaLoginDoIxc(r, ctx = {}) {
...
function calcStatusContrato(statusContrato, statusAcesso) {
...
function agregarStatusCliente(contratosDoCliente) {
...
// ----------------------------------------------------------------------------
// CommonJS de propósito, e não ESM. Os arquivos de `/api` são compilados de ESM
// para CommonJS na Vercel, mas o que mora FORA de `/api` é embarcado como está.
// Um módulo ESM aqui viraria alvo de um `require()` em produção e a função
// morreria na primeira invocação com ERR_REQUIRE_ESM — inclusive o
// /api/atendimento, que também importa daqui. Em CommonJS os dois lados
// funcionam: o `require()` do código compilado e o `import { ... }` do Node,
// que lê os nomes deste module.exports.
// ----------------------------------------------------------------------------
module.exports = {
  emLotes,
  ixcNumero,
